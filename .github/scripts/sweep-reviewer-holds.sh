...
# approve step (claude-review.yaml's `thread_resolve` job) never re-runs for
# that PR.
||||||| base
# with no follow-up push emits no workflow-triggering event, so the per-push
...
#
# `gh pr list --json` asks GraphQL, and a GraphQL 5xx took the whole weekly sweep
# red once already: the listing is the first call, so an outage there loses every
# PR rather than one. retry_stdout re-tries with backoff and emits only the
# succeeding attempt, so a real fault still exhausts the cap and goes red.
