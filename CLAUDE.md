...
- **Write every summary, PR body, issue, review reply, and shipped doc in ASD-STE100 Simplified Technical English.** Scope: chat summaries and reports, PR descriptions, issue titles and bodies, review-thread replies, `README.md`, and `changelog.d/` fragments, which ship to consumers through `CHANGELOG.md`. PR titles keep the `conventional-commits` subject form instead. The rules also bind any hook `description:` or module docstring **you edit** — bring that one to these rules while you are in it. They do not oblige a sweep of the ones you do not touch. Several shipped descriptions still carry a sentence above 25 words, and that is deliberate: a rule that grandfathers a baseline it never pays down is the guard smell this pack exists to catch, so this one pays its baseline down one edit at a time instead of claiming a clean tree it does not have. STE’s Part 2 approved-word dictionary is not in your context, so these Part 1 rules bind:
  - **One meaning per word, one word per meaning.** Pick the plainest ordinary word and reuse it.
  - **One idea per sentence — ≤20 words for an instruction, ≤25 for a description.** Split a clause chain; do not comma-join it.
  - **Active voice, simple present or past, explicit subject; imperative for instructions.** “The check blocks the merge”, not “the merge is blocked”; “landed”, not “has landed”. No `-ing` form doing a verb’s job, and keep the articles and relative pronouns: “the check that blocks the merge”, never “check blocking merge”.
  - **At most 3 words in a noun cluster.** Break a longer one with prepositions: “merge queue slot pool exhaustion” → “the merge queue has run out of slots”.
  - **One topic per paragraph, ≤6 sentences, topic sentence first.** A set of conditions or steps becomes a vertical list.
  - **Say it positively unless it is a warning; no idioms, metaphors, or slang.** “Run X after Y” beats “do not run X before Y”. Jokes in chat are fine.
  - **Verbatim spans are exempt:** commands, code, paths, identifiers, test names, quoted tool output, and the technical nouns they force. A Markdown table is exempt as a _structure_; the prose inside its cells is not.
- **When you EXPLAIN — a tradeoff, a mechanism, a failure, why an approach is wrong — write for a reader who has never seen this repo.** Your own sense of having explained it is not evidence: you hold the thing, so the compressed version feels complete to you and arrives as noise. Name the concrete thing before the term for it (“the file that says which base image to start from”, then `Dockerfile`). Say what breaks and for whom, not the mechanism’s name. Give one worked case with real numbers before any generalization. This grants ONE carve-out from the no-metaphors rule above: a concrete analogy for an unfamiliar mechanism (a fingerprint for a digest). Introduce it once, then use the plain term, and never let an analogy replace the fact it illustrates.
- **Never hand-wrap prose — one paragraph is ONE line** in a file Prettier owns (`README.md`, `CLAUDE.md`, `changelog.d/*.md`). Prettier reflows them, so your own newlines only make a one-word edit re-diff the whole paragraph. Prose only: leave fenced code, tables, and YAML alone.
...
- **A lint that asks a question about shell STRUCTURE parses the grammar (`_cts_bash_ast`), never the text.** Is this a command or a string a command prints? An argument or a `file_redirect`? One command or two? Does this `;` separate anything, inside quotes? A regex, a quote-state scanner, a segment splitter, or `shlex` answering any of those is a re-implementation of bash that gets a different subset of real shell wrong — `check-versionless-install` shipped four false positives in a day that way. Full rule, the tells, and the audit of which lints still scan text: [`.claude/rules/shell-lint-parsing.md`](.claude/rules/shell-lint-parsing.md).
...
- **A pre-flight cap that skips work is a silent failure, and the “Fail loudly” and “Let exceptions propagate” bullets above do not reach it.** They govern an error that already exists: a raise, a non-zero exit, a failed post-condition. A cap raises nothing — it measures a resource first, declines the work, and exits 0, so there is no error to swallow. Two things make it safe, and both belong to the caller rather than the cap. The cap must make its skip visible to whatever depends on the work. The consumer must then branch on that signal, because a consumer that ignores it turns the cap into a green run over absent inputs. `prepare-pr-review-input.sh` caps a diff at 20,000 lines, writes `oversized-notice.txt`, and emits `oversized=true`; its step in `claude-review.yaml` carries no `id:`, so nothing can read that output, and the next step gates on `has_threads` instead. Over the cap the notice is never posted and the judge reads a `diff.txt` that was never written. Wire the consumer in the same change, or delete the cap and let the resource fail loudly.
