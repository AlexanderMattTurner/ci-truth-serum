...
# `cg_*`, a common status-message helper naming, exactly as check_exit_suppression does.
...
# options — so pipefail is active at RUNTIME even though no `set -o pipefail` appears in the
# file. Treating it as pipefail-scoped is what catches the SIGPIPE trap in a sourced lib's
# teardown credential check, which the in-file-only heuristic would miss.
