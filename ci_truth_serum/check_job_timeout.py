...
def _block_opted_out(block_text: str) -> bool:
    ...
    The lines come from `yaml_script_view`, which keeps the two places a job may
    carry a marker: a real YAML comment, and a `run:` value in any style, where a
    ...
    """
    return any(annotated(line, ALLOW) for line in yaml_script_view(block_text))
