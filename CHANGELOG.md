...
- `check-workflow-pipefail` and `check-flag-arity` now parse shell with a real bash grammar (`tree-sitter-bash`) instead of hand-rolled quote/heredoc/case tokenizers, closing the blind spots those approximations had. `check-workflow-pipefail` now sees a masking pipe that an escaped quote (`echo "a\"" | tee`), a `
