...
| `check-unreferenced-local-uses`    | A local composite action, or a workflow whose only trigger is `workflow_call`, that no `uses:` reaches. It never runs, and it still reads to the next author as a live code path. Opt out with `# unreferenced-ok: <reason>` naming the caller in another repository.     | a composite action whose last caller went away                      |
