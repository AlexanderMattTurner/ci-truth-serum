...
| `check-pending-cancel-concurrency` | A required check went **red on the live head with no real failure**: `on.pull_request.types` included a type beyond opened/synchronize/reopened (e.g. `labeled`—a Dependabot PR is born with labels, so `opened` + several `labeled` runs queue on ONE head SHA), and a per-ref/per-PR `concurrency.group` let GitHub's one-running + one-pending slot cancel a **same-SHA sibling**, whose `always()` reporter resolved `cancelled`. Checked at the workflow level and on every job; `cancel-in-progress` can't help (either value kills a current-SHA run). Fix: drop the group or key it on `github.run_id`. Opt out with `# pending-cancel-ok`. |
...
| `check-stray-tool-markup`    | An agent leaked the trailing scaffolding of a file-authoring tool call — a bare closing `content`/`invoke` tag (and the `antml:`-prefixed variants) — onto its own line in a committed doc, where it rendered as literal garbage that only a human caught. Flags a line that is _entirely_ a stray tool-call tag (`</invoke>`, an opening `invoke`/`parameter` tag, `<function_calls>`, a bare closing `content` tag); inline mentions, inline-code spans, and syntax shown inside a fenced code block are never flagged. Suppress a genuine case with `allow-stray-markup: <reason>` on the line above.                                                            |
