...
| `check-required-reporter`          | A new reporter is green but nobody added it to branch protection, so it gates nothing. Reads the `# required-check:` markers. A required matrix job that can skip fails too: its per-combination check names never report, so the branch blocks forever with nothing red.                       | a reporter with no `# required-check:` marker                           |
...
The lint rejects that shape too. Make the job unconditional and gate its steps instead. Or add a sibling job that runs on `if: always()` and carries the same `name:` template.
