...
| `check-curl-retry`                 | A single-shot `curl -o file` fails the whole install on one dropped packet. curl must carry its own `--retry`, plus `--retry-all-errors` so a refused connection is retried too.                                                                                                                                                                                                                                                                                                                                                                                                                | `curl -fsSL -o uv.tar.gz "$url"` with no `--retry-all-errors`           |
