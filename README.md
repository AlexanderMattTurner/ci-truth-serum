...
| `check-required-reporter`          | A new reporter is green but nobody added it to branch protection, so it gates nothing. Reads the `# required-check:` markers. A required matrix job that can skip fails too: its per-combination check names never report, so the branch blocks forever with nothing red. Opt out with `# matrix-context-ok: <reason>`. A required job whose `name:` holds a non-matrix `${{ }}` fails too: the sync registers text that no run reports. | a reporter with no `# required-check:` marker                           |
