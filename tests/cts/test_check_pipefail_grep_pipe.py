...
# --- the pipeline: last stage is a quiet grep, fed by a streaming producer ---
@pytest.mark.parametrize(
    "line",
    [
        # the exact class that ships the bug
        "secret_store ls | grep -q github",
        'container ls 2>/dev/null | grep -qF "$name"',
        "egress_allow_rules | grep -qF host",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        'grep -q pat <<<"$var"',
        'grep -qF "$name" <<<"$(container ls 2>/dev/null)"',
...
def test_sourced_bash_lib_is_pipefail_scoped() -> None:
    ...
    in-file `set -o pipefail` — this is what catches a sourced lib's teardown credential
    check."""
...
def test_own_shell_surface_is_clean() -> None:
    """Every tracked shell file in this repo must pass — a new `producer | grep -q`
    under pipefail here turns this red, proving the check is wired to real sources.

    Driven from `git ls-files` so it scans exactly the TRACKED files pre-commit
    operates on, filtered to the hook's `types: [shell]` selection."""
    ...
    assert offenders == [], f"pipefail `| grep -q` in own shell surface: {offenders}"
