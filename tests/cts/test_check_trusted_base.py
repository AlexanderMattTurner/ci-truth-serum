...
def test_a_marker_in_a_run_script_suppresses(tmp_path):
    ...
    author writes the marker beside the step that spends the write permission."""
    path = _write(
        tmp_path,
        "on:\n  pull_request:\n"
        "permissions:\n  contents: write\n"
        ...
        + _HEAD_CHECKOUT
        + "      - run: |\n"
        "          # trusted-base-ok: the token only writes a cache key\n"
        "          push-cache\n",
    )
    assert tb.check_file(path) == []
...
def test_a_marker_in_a_quoted_one_line_run_suppresses(tmp_path):
    ...
    path = _write(
        ...
        "jobs:\n  build:\n    runs-on: ubuntu-latest\n"
        + _HEAD_CHECKOUT
        + "      - run: 'push-cache  # trusted-base-ok: writes a cache key only'\n",
    )
    assert tb.check_file(path) == []
