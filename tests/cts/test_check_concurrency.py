...
def test_own_ci_workflow_passes():
    """ci-truth-serum's own CI workflow sets cancel-in-progress explicitly and keys
    its concurrency group per-ref, so the repo dogfoods its own lint. Scoped to the
    product's workflow (not the inherited template's, which template-sync may rewrite
    independently)."""
