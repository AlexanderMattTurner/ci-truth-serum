...
_SRC = HOOKS_DIR / "check_historical_comments.py"
mod = load_hook("check_historical_comments.py", "check_historical_comments")


def test_every_marker_fires() -> None:
    """One case per SSOT member — a dropped marker is invisible to line coverage,
    so drive the test from the list itself."""
...
def test_script_accepts_clean_and_annotated(tmp_path: Path) -> None:
    """Negative control: present-tense comments and an annotated one pass (exit 0)."""
...
def test_enforced_scope_is_clean() -> None:
    ...
    in scope turns this red, proving the check is wired to real sources.
    Non-vacuous: the firing cases above show `violations` is not constant-empty."""
    exclude = dogfood_extras_exclude()
    kinds = {"shell", "python", "javascript", "ts"}
    ...
    for rel in tracked:
        path = REPO_ROOT / rel
        if not rel or exclude.match(rel) or not path.is_file():
            continue
