...
_SRC = HOOKS_DIR / "check_graceful_handwave.py"
mod = load_hook("check_graceful_handwave.py", "check_graceful_handwave")
...
def test_both_word_forms_fire_in_a_comment(word: str) -> None:
    ...
    member: a regex making only the trailing `y` optional would let "graceful"
    sail through — this case is what catches that."""
...
def test_prose_flag_forces_prose_mode(tmp_path: Path) -> None:
    """A code-less text file is scanned comment-only (and stays silent) without
    the flag; --prose scans its every line."""
...
def test_prose_doc_level_annotation_excuses_whole_document(tmp_path: Path) -> None:
    """A --prose document (e.g. a PR title+body) makes one argument as a whole:
    one allow-graceful line anywhere excuses it — a document ABOUT the word could
    never satisfy per-line annotation."""
    body = tmp_path / "pr-body"
    body.write_text(
...
def test_txt_is_not_prose(tmp_path: Path) -> None:
    """.txt is data (a wordlist can carry "graceful" as a dictionary word) — it
    must not be scanned as prose."""
...
def test_main_returns_1_and_names_the_file_on_a_violation(tmp_path, capsys) -> None:
    """A code file (.sh -> CODE mode) with an un-annotated comment hit: main
    returns 1 and prints `<path>:<line>: <message>` to stderr."""
    f = tmp_path / "x.sh"
...
def test_main_skips_an_unreadable_path(tmp_path) -> None:
    """An absent path raises OSError in main's read — skipped, not fatal."""
...
def test_main_prose_flag_doc_level_annotation_excuses_the_document(tmp_path) -> None:
    ...
    excuses it, so main returns 0 despite the word."""
...
def test_main_prose_flag_flags_an_unannotated_document(tmp_path, capsys) -> None:
    """--prose with no annotation scans every line of the .txt (which is NOT
    prose by suffix) as prose, so the word fires and main returns 1."""
    f = tmp_path / "PR_BODY.txt"
...
def test_enforced_scope_is_clean() -> None:
    ...
    one authoritative skip list) passes today — a regression reintroducing the
    word un-annotated fails here before CI. Non-vacuous: the firing cases above
    show `violations` is not constant-empty."""
    exclude = dogfood_extras_exclude()
    kinds = {"shell", "python", "javascript", "ts", "markdown", "rst"}
    ...
    scanned = [
        ...
        for rel in tracked
        if rel
        and not exclude.match(rel)
        and (REPO_ROOT / rel).is_file()
        and identify.tags_from_path(str(REPO_ROOT / rel)) & kinds
    ]
